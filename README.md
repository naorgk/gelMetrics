...
3. Run script read_tables.m on CSV files generated in step 2. The script should be in the same folder as CSV files. The script expects the specific structure of the Mosaic Particle Tracker.
4. Run script Collect_intensity_data.m on microscopy images and .mat files generated in step 3. The script assumes the same alphabetical order for both .tif and .mat files. The script will generate two .mat files corresponding to data & background.
5. Run script ExperimentalAnalysis.m on .mat files generated in step 4.
